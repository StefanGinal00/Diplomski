...
func _run() -> void:
	...
	shaft.get_node("HollowUpperDoor")._on_body_entered(player)
	...
	lower_door._on_body_entered(player)
	await create_timer(0.5).timeout
	...
	_check(hollow.has_node("Cache_hollow_resonance") and hollow.has_node("hollow_echo_wisp"), "Awakened Hollow cache or encounter missing")
	...
	_check("2/6 PLAYABLE ROOMS" in game.get_node("UI").map_route_label.text and "CACHES 1/11" in game.get_node("UI").map_route_label.text, "Map did not track Hollow room and cache")
